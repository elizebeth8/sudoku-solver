...
public class SudokuGUI extends JFrame{
    ...
    private final JButton solveButton = new JButton("Solve it");
    ...
    public SudokuGUI(){
        ...
        solveButton.addActionListener(e->new Thread(()->{//whatthefuck
            ...
            for(int col = 0;col < GRID_SIZE;col++){
                if(board[row][col]==0){
                    for(int number=1;number <= GRID_SIZE;number++){
                        int numToTry = (number + random.nextInt(GRID_SIZE))% GRID_SIZE + 1;//DIDNT GET
    ...
    private boolean solveBoard(int[][] board, boolean firstCall){
        ...
        if (firstCall) {
            SwingUtilities.invokeLater(this::updateBoard);   //didnt get
        }
        return true;
    }
    ...
    private void updateCell(int row, int col, int numToTry){
        SwingUtilities.invokeLater(() -> {
            ...
            cells[row][col].repaint();  //didnt get
        });
        try {
            Thread.sleep(100);
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();   //didnt get
        }
    }
    ...
    private boolean isValidPlacement(int[][] board, int number, int row, int col){
        return !isNumberInRow(board, number, row) && !isNumberInColumn(board, number, col) && !isNumberInBox(board, number, row, col);
    }
